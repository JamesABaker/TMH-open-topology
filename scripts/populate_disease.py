...
def var_to_database(
    ...
    if var_record_location == "-":
        print(
            "Unkown sequence location. Possibly intron: ",
            uniprot_record,
            var_record_location,
            aa_wt,
            "->",
        ...
        print(
            "Wildtype amino acid not defined. Assuming this is not an SNP: ",
            uniprot_record,
            var_record_location,
            aa_wt,
            "->",
        ...
        print(
            "More than a single residue changed. Assuming this is not an SNP: ",
            uniprot_record,
            var_record_location,
            aa_wt,
            "->",
        ...
        print(
            "Stop codon introduced. This will change more than one residue:",
            uniprot_record,
            var_record_location,
            aa_wt,
            "->",
        ...
        try:
            ...
            if int(var_record_location) <= len(str(protein.full_sequence)):
                    ...
                    for var in disease_variants:
                        for dis in diseases:
                            (
                                record_for_database,
                                created,
                            ) = Disease.objects.get_or_create(disease_name=dis)
                            disease_type = Disease.objects.get(disease_name=dis)
                            disease_type.implicated_variants.add(var)
                            print(f"Added {var} to {dis}")
                else:
                    print(
                        "Mismatch between wild-type amino acids. UniProt:",
                        str(residue_variant.amino_acid_type),
                        str(variant_source),
                        ":",
                        str(aa_wt),
                        "for record",
                ...
                print(
                    "Variant position exceeds the length of the protein. Protein length:",
                    len(str(protein.full_sequence)),
                    "Variant position:",
                    var_record_location,
                    "for record",
                    uniprot_record,
...
def varmap_process(varmap_list, source, varmap_index):
    for varmap_item in varmap_list:
        ...
        if (
            len(varmap_item[varmap_index["AA_CHANGE"]]) == 3
            and "/" in varmap_item[varmap_index["AA_CHANGE"]]
        ):
...
def run():
    ...
    random.shuffle(
        clinvar_results_list
    )  # Increases intial speed for rapid population serialisation.
    print(varmap_header_dict(varmap_files["clinvar"]))
    print(clinvar_results_list[0])
    varmap_process(
        clinvar_results_list, "ClinVar", varmap_header_dict(varmap_files["clinvar"])
    )
