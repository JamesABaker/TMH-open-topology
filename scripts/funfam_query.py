...
for i in ff:
    variants=Variant.objects.filter(disease_status="d",variant_source="ClinVar", residue__funfamresidue__funfam__funfam_id=i.funfam_id).distinct("pk")
    ...
    for a_protein in protein:
        ...
        info.append(variants.filter(residue__protein__uniprot_id=a_protein.uniprot_id).distinct().count())
        keys=Keyword.objects.filter(proteins__uniprot_id=a_protein.uniprot_id).distinct()
        kw=[]
        for x in keys:
            kw.append(x.keyword)
        info.append(kw) 
