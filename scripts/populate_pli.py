        ...
        for line_number, pli_entry in enumerate(inputfile):
            if line_number > 0:
                ...
                if "NA" not in pli and "NA" not in missense: 
                    database_update(marcia_uniprot_id=uniprot, pli_score=pli, missense_score=missense)

def database_update(marcia_uniprot_id=None, pli_score=None, missense_score=None):
    Protein.objects.filter(uniprot_id=marcia_uniprot_id).update(oe_mis_upper_gn=missense_score, pLI_gn=pli_score)

def run():
    pli_parser("scripts/external_datasets/ensembl_uniprot_MANE_metrics-ALL-07-10-2020_simple_uniprotSPC") 
