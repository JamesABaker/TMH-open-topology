    ...
    with open(varmap_file, encoding="ISO-8859-1") as inputfile:
        ...
        while varmap_line:
            ...
            if cnt == 1:
                ...
                uniprot_accession = clean_query(
                    str(varmap_line[uniprot_accession_index])
                )
                ...
                if str(uniprot_accession) in input_query_set:
                    ...
                    uniprot_record = clean_query(
                        varmap_line[varmap_headers["UNIPROT_ACCESSION"]]
                    )
                    variant_review = "Unknown"
                    aa_wt = varmap_line[varmap_headers["UNIPROT_AA"]]
                    if (
                        len(varmap_line[varmap_headers["AA_CHANGE"]]) == 3
                        and "/" in varmap_line[varmap_headers["AA_CHANGE"]]
                    ):
                    ...
                    var_to_database(
                        ...
                        variant_source,
                        user_id,
                        variant_maf=parsed_af,
                        variant_qc=parsed_qc,
                    )
                    # print(cnt, ":")
                    proteins.append(uniprot_record)
            varmap_line = inputfile.readline()
    print(
        f"{len(proteins)} protein variants added, {len(set(proteins))} unique proteins"
    )
    return ()
...
def id_parse(varmap_id):
    ...
    print(varmap_id)
    varmap_order = {0: "rsid", 1: "qc", 2: "allele_count", 3: "allele_frequency"}
