...
def basic_num():
    ...
    tmh_residue_num = Residue.objects.filter(tmh_residue__feature_location="TMH").distinct('pk').count()
    print("TMH residues,", tmh_residue_num)

    for i in residues:
        count=Residue.objects.filter(amino_acid_type=i, tmh_residue__feature_location="TMH").distinct('pk').count()
        print(i, ", ", count)

    #flanks
    flank_residue_num = Residue.objects.filter(flank_residue__feature_location="Inside flank").distinct('pk').count()
    print("inside flank residues,", flank_residue_num)

    for i in residues:
        count=Residue.objects.filter(amino_acid_type=i, flank_residue__feature_location="Inside flank").distinct('pk').count()
        print(i, ", ", count)

    flank_residue_num = Residue.objects.filter(flank_residue__feature_location="Outside flank").distinct('pk').count()
    print("outside flank residues,", flank_residue_num)

    for i in residues:
        count=Residue.objects.filter(amino_acid_type=i, flank_residue__feature_location="Outside flank").distinct('pk').count()
        print(i, ", ", count)
    ...
    for i in residues:
        count=Residue.objects.filter(amino_acid_type=i, flank_residue=None, tmh_residue=None).distinct('pk').count()
        print(i, ", ", count)



    structural_residue_num = Residue.objects.exclude(structural_residue=None).distinct('pk').count()
    print("Residues with a map to at least one residue in a structure,", structural_residue_num)
    non_structural_residue_num = Residue.objects.filter(structural_residue=None).distinct('pk').count()
    ...
    uniprot_tmh_boundary = Tmh.objects.filter(tmh_evidence='UniProt').distinct('pk').count()
    ...
    mptopo_tmh_boundary = Tmh.objects.filter(tmh_evidence='MPTOPO').distinct('pk').count()
    print("MPTOPO TMH boundaries,", mptopo_tmh_boundary)

    print("\n\nVariants\n")
    # Complex query example. How many variants are in the TMH?
    g_variants_num = Variant.objects.filter(variant_source='gnomAD').distinct('pk').count()
    print("gnomAD variants,", g_variants_num)
    tmh_g_variants_num = Variant.objects.filter(residue__tmh_residue__feature_location = "TMH", variant_source='gnomAD').distinct('pk').count()
    print("TMH gnomAD variants,", tmh_g_variants_num)

    inside_flank_g_variants_num = Variant.objects.filter(residue__flank_residue__feature_location="Inside flank").filter(variant_source='gnomAD').distinct('pk').count()
    print("Inside flank gnomAD variants,", inside_flank_g_variants_num)

    outside_flank_g_variants_num = Variant.objects.filter(residue__flank_residue__feature_location="Outside flank").filter(variant_source='gnomAD').distinct('pk').count()
    print("Outside flank gnomAD variants,", outside_flank_g_variants_num)

    non_tmh_g_variants_num = Variant.objects.filter(residue__tmh_residue=None, residue__flank_residue=None, variant_source='gnomAD').distinct('pk').count()
    ...
    tmh_d_variants_num = Variant.objects.filter(residue__tmh_residue__feature_location = "TMH", disease_status='d').distinct('pk').count()
    print("TMH disease variants,", tmh_d_variants_num)

    flank_d_variants_num = Variant.objects.exclude(residue__tmh_residue=None, residue__tmh_residue__feature_location = "TMH").filter(disease_status='d').distinct('pk').count()

    inside_flank_d_variants_num = Variant.objects.filter(residue__flank_residue__feature_location="Inside flank").filter(disease_status='d').distinct('pk').count()
    print("Inside flank disease variants,", inside_flank_d_variants_num)

    outside_flank_d_variants_num = Variant.objects.filter(residue__flank_residue__feature_location="Outside flank").filter(disease_status='d').distinct('pk').count()
    ...
    d_variants_humsavar_num = Variant.objects.filter(disease_status='d', variant_source="Humsavar").distinct('pk').count()
    print("Humsavar disease variants,", d_variants_humsavar_num)
    print("TMH Humsavar disease variants,", tmh_d_variants_humsavar_num)
    non_tmh_d_variants_num = Variant.objects.filter(residue__tmh_residue=None, residue__flank_residue=None).filter(disease_status='d').distinct('pk').count()
    print("Non-TMH disease variants,", non_tmh_d_variants_num)
    non_tmh_d_variants_clinvar_num = Variant.objects.filter(residue__tmh_residue=None, residue__flank_residue=None, disease_status='d', variant_source="ClinVar").distinct('pk').count()
    print("Non-TMH ClinVar disease variants,", non_tmh_d_variants_clinvar_num)
    non_tmh_d_variants_humsavar_num = Variant.objects.filter(residue__tmh_residue=None, residue__flank_residue=None, disease_status='d', variant_source="Humsavar").distinct('pk').count()
