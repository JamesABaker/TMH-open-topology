...
def statistics(list_one=[], list_two=[]):
    ...
    score, pvalue=stats.fisher_exact([list_one, list_two])
...
def query_to_scores(queryset_of_proteins):
    ...
    pli_average=averages(plis)
    pli_above_below=pli_threshold(plis)
    return([pli_average, pli_above_below[0], pli_above_below[1]])
...
def run():
    ...
    print(f'Non-TMPs,{ref_scores[0]},{ref_scores[1]},{ref_scores[2]}, {statistics([ref_scores[1], ref_scores[2]],[ref_scores[1], ref_scores[2]])}')
    ...
    print(f'Multipass,{scores[0]},{scores[1]},{scores[2]}, {statistics([scores[1], scores[2]],[ref_scores[1], ref_scores[2]])}')

    query=Protein.objects.filter(total_tmh_number=1).distinct('uniprot_id')
    scores=query_to_scores(query)
    print(f'Singlepass,{scores[0]},{scores[1]},{scores[2]}, {statistics([scores[1], scores[2]],[ref_scores[1], ref_scores[2]])}')

    query=Protein.objects.filter(total_tmh_number__gte=1, keywords__keyword="G-protein coupled receptor").distinct('uniprot_id')
    scores=query_to_scores(query)
    print(f'GPCRs,{scores[0]},{scores[1]},{scores[2]}, {statistics([scores[1], scores[2]],[ref_scores[1], ref_scores[2]])}')

    query=Protein.objects.filter(total_tmh_number__gte=1, keywords__keyword="Ion channel").distinct('uniprot_id')
    scores=query_to_scores(query)
    print(f'Ion channel,{scores[0]},{scores[1]},{scores[2]}, {statistics([scores[1], scores[2]],[ref_scores[1], ref_scores[2]])}')
    ...
    scores=query_to_scores(query)
    print(f'Transport excluding ion channels,{scores[0]},{scores[1]},{scores[2]}, {statistics([scores[1], scores[2]],[ref_scores[1], ref_scores[2]])}')

    query=Protein.objects.filter(keywords__keyword="Kinase").distinct('uniprot_id')
    scores=query_to_scores(query)
    print(f'Kinase,{scores[0]},{scores[1]},{scores[2]}, {statistics([scores[1], scores[2]],[ref_scores[1], ref_scores[2]])}')
