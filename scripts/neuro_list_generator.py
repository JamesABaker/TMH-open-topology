...
def binding_check(variant_database_object):
    binding_residues = Residue.objects.filter(
        variant=variant_database_object, binding_residue=True)
...
def tmh_check(variant_database_object):
    tmh = Tmh.objects.filter(
        meta_tmh=True, tmh_residue__residue__variant=variant_database_object)
    if tmh.count() > 0:
        return("Tmh")
    else:
        return(None)
...
def file_output(variant_object, neuro_status):
    ...
    physico_status = physico_change(
        variant_object.aa_wt, variant_object.aa_mut)
...
def run():
    variant_objects = Variant.objects.filter(
        variant_source="ClinVar", disease_status="d").distinct('pk')
